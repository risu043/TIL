...
- 翻訳用の全ポケモンデータをページロードの度ににリクエストしている。jsonファイルにしてローカルで管理するべき
...
検索機能にsearchParamsを使用、検索時のjsonファイルと条件分岐追加<br>
検索毎に1302匹すべてfetchしているのが非効率なので改善したいがpokeAPIの都合でむずかしそう。<br>
ポケモンの亜種は元のポケモンと和名・英名ともに同じ。図鑑番号でfetchできるのは1026匹目まで。
